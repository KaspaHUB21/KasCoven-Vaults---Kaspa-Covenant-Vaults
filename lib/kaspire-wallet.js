...
const METADATA_SESSION_VERSION = "2";
const METADATA_SESSION_STORAGE_KEY = "kascoven:kaspire-metadata-version";
...
const KASPIRE_WAKE_LINK =
  "intent://dapp#Intent;scheme=kaspire;" +
...
async function getClient() {
  ...
  if (!clientPromise) {
    clientPromise = import("@walletconnect/sign-client").then(async ({ default: SignClient }) => {
      ...
      if (window.localStorage.getItem(METADATA_SESSION_STORAGE_KEY) !== METADATA_SESSION_VERSION) {
        for (const session of client.session.getAll()) {
          await client.disconnect({
            topic: session.topic,
            reason: { code: 6000, message: "KasCoven WalletConnect metadata was upgraded." },
          }).catch(() => null);
        }
        for (const pairing of client.pairing.getAll()) {
          await client.core.pairing.disconnect({ topic: pairing.topic }).catch(() => null);
        }
        window.localStorage.setItem(METADATA_SESSION_STORAGE_KEY, METADATA_SESSION_VERSION);
      }
      const refresh = () => {
...
async function requestKaspire(method, params = {}) {
  ...
  // Publish the WalletConnect request before offering to switch applications.
  // Otherwise a fast tap can foreground Kaspire before the relay has received
  // the request, leaving the wallet with nothing to approve.
  const outcomePromise = client.request({
...
export async function connectKaspire({ onDisplayUri } = {}) {
  const [client, QRCode] = await Promise.all([getClient(), import("qrcode")]);
  ...
  if (!usableSession(activeSession)) {
    ...
    onDisplayUri?.({ appLink, intentLink, qrDataUrl });
