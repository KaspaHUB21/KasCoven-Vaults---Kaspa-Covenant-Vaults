...
function appMetadata() {
  ...
  const metadata = {
    name: recoveryMode ? "KasCoven Recovery Tool" : wizardMode ? "KasCoven Wizard" : "KasCoven Vaults",
    description: recoveryMode
      ? "Recover KasCoven vaults from Kaspa on-chain data"
      : wizardMode
        ? "Permissionless first-come-first-served Kaspa prize vaults"
      : "Time-locked covenant vaults on Kaspa",
    url: origin + (recoveryMode ? "/recovery/tool" : wizardMode ? "/wizard" : "/"),
    icons: [origin + "/kascoven-logo.png"],
  };
  if (!["localhost", "127.0.0.1", "::1"].includes(window.location.hostname)) {
    metadata.redirect = { universal: metadata.url };
  }
  return metadata;
}
