...
export async function connectKaspire({ onDisplayUri } = {}) {
  ...
  if (!usableSession(activeSession)) {
    ...
    const pairingDetails = { appLink, intentLink, qrDataUrl: "" };
    // Android can launch immediately; QR generation is intentionally deferred
    // because loading the QR module used to delay the deep link by several seconds.
    onDisplayUri?.(pairingDetails);
    ...
    onDisplayUri?.({ ...pairingDetails, qrDataUrl });
