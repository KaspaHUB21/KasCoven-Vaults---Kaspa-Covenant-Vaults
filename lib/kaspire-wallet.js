...
const METADATA = {
  ...
  description: "Time-locked covenant vaults on Kaspa",
  url: "https://vaults.kaslab.space",
  icons: ["https://vaults.kaslab.space/deadmansswitch.png"],
  redirect: { universal: "https://vaults.kaslab.space" },
};
...
async function getClient() {
  ...
  if (!clientPromise) {
    clientPromise = import("@walletconnect/sign-client").then(async ({ default: SignClient }) => {
      const client = await SignClient.init({ projectId: PROJECT_ID, metadata: METADATA });
      const refresh = () => {
